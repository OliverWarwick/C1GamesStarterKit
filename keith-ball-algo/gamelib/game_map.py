...
class GameMap:
    ...
    def move_unit(self, unit, location, player_index=0):
        ''' 
        For adding a unit which already exists on the map moving from somewhere else.
        '''
        if not self.in_arena_bounds(location):
            self._invalid_coordinates(location)
        if player_index < 0 or player_index > 1:
    ...
    def remove_one_unit(self, location, unit):
        '''Remove one unit on the map in the given location.

        Args:
            location: The location that you will empty of units
        Returns:
            boolean: Whether was successful
        '''
